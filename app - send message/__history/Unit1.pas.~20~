...
begin
  ...
  try
    ...
    notificacao.Title := 'Atenção: você tem uma nova mensagem';
...
begin
  ...
  try
    ...
    notificacaoA.Title := 'Atenção: você tem uma nova mensagem';
  ...
  try
    ...
    notificacaoB.Title := 'Atenção: você tem uma nova mensagem';
...
procedure TForm1.NotificationCenter1ReceiveLocalNotification(Sender: TObject;
  ANotification: TNotification);
begin
  lbMensagens.Items.Add('Notificação recebida: ' + ANotification.Name);
end;
